...
# --------------------------- 日志工具 ---------------------------
LOG_DIR = "/home/lixingfeng/UniAD_exmaine/UniV2X/univ2x_purned/modelopt/logs_all_model_prune_backbone_neck_bycodex"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="none",
        help="分布式启动方式；score_func 需与 tools/test.py 对齐时应使用非 none。",
    )
    parser.add_argument("--local_rank", "--local-rank", type=int, default=0)
    parser.add_argument(
        "--tmpdir",
        default=None,
        help="分布式评估时用于收集结果的临时目录。",
    )
    parser.add_argument(
        "--gpu-collect",
        action="store_true",
        help="分布式评估时使用 GPU 收集结果。",
    )
...
# --------------------------- score func ---------------------------
@torch.no_grad()
def _eval_car_mean_ap(model_ddp: MMDistributedDataParallel, dataset, data_loader,
                      eval_kwargs: Dict[str, Any], tmpdir: str = None,
                      gpu_collect: bool = False) -> float:
    rank, world_size = get_dist_info()
    print(f"[score_func][rank {rank}] >>>>>> ENTERING _eval_car_mean_ap, "
          f"dataset size={len(dataset)}, loader batches={len(data_loader)}, "
          f"world_size={world_size} <<<<<<", flush=True)
    model_ddp.eval()
    outputs = custom_multi_gpu_test(model_ddp, data_loader, tmpdir, gpu_collect)

    score_tensor = torch.tensor(float("nan"), device=f"cuda:{torch.cuda.current_device()}")
    if rank == 0:
        ...
        if bbox_outputs is None:
            raise RuntimeError("custom_multi_gpu_test 返回结果为空，无法评估。")
...
def build_score_func(dataset, val_loader, eval_kwargs: Dict[str, Any],
                     tmpdir: str = None, gpu_collect: bool = False):
    _call_count = [0]  # mutable counter in closure

    def score_func(model: nn.Module) -> float:
        if not dist.is_available() or not dist.is_initialized():
            raise RuntimeError(
                "score_func 已切换为分布式评估。请使用 torch.distributed 启动脚本，"
                "例如 --launcher pytorch。"
            )
        _call_count[0] += 1
        rank, _ = get_dist_info()
        print(f"\n{'='*60}", flush=True)
        print(f"[score_func][rank {rank}] >>>>>> CALLED (#{_call_count[0]}) <<<<<<", flush=True)
        ...
        model_ddp = MMDistributedDataParallel(
            real_model,
            device_ids=[torch.cuda.current_device()],
            broadcast_buffers=False,
        )
        score = _eval_car_mean_ap(model_ddp, dataset, val_loader, eval_kwargs, tmpdir, gpu_collect)
...
def main():
    ...
    if not distributed:
        raise RuntimeError(
            "该 bycodex 版本已将 score_func 对齐到 tools/test.py 的分布式评估路径。"
            "请使用分布式方式启动，例如 --launcher pytorch。"
        )
    ...
    print(f"[INFO][rank {rank}/{world_size}] 使用设备: {device}")
