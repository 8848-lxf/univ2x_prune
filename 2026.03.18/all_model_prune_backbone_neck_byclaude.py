...
def fixed_head_indices(dataset, frames_per_scene=10, sampling_mode="skip_warmup"):
    ...
    sampling_mode:
      - "head":        从第 1 帧（index 0）开始取
      - "skip_warmup": 从第 5 帧（index 4）开始取，跳过 BEV 历史队列预热期
    """
...
# --------------------------- 日志工具 ---------------------------
LOG_DIR = "/home/lixingfeng/UniAD_exmaine/UniV2X/univ2x_purned/modelopt/all_model_prune_backbone_neck_byclaude/logs_all_model_prune_backbone_neck_byclaude"
...
class _Tee:
    """Python 层 tee：终端和日志文件同时输出所有内容（不过滤任何信息）。

    所有 stdout/stderr 输出（包括进度条、异常信息、score_func 调试打印）
    全部同时写入终端和日志文件，确保日志完整可回溯。
    """
...
def _install_excepthook(log_fh):
    """安装全局异常钩子，确保未捕获异常的完整 traceback 写入日志。"""
...
def setup_logging() -> str:
    ...
    # 安装异常钩子，确保 traceback 写入日志
    _install_excepthook(log_fh)
...
def parse_args():
    ...
    parser.add_argument(
        "--out-dir",
        default="/home/lixingfeng/UniAD_exmaine/UniV2X/univ2x_purned/modelopt/all_model_prune_backbone_neck_byclaude/output",
    ...
    parser.add_argument(
        ...
        default="skip_warmup",
        help="校准集连续采样模式：head=从第1帧取，skip_warmup=从第5帧取（跳过BEV预热期）。",
    )
    ...
    parser.add_argument(
        ...
        choices=["head", "skip_warmup"],
        default="skip_warmup",
        help="评分集连续采样模式：head=从第1帧取，skip_warmup=从第5帧取（跳过BEV预热期）。",
    )
    parser.add_argument(
        "--params-percent",
        type=str,
...
def build_subset_loader(cfg, split="val", subset_mode="single_scene",
    ...
    """
    构建固定子集的 DataLoader。
      - per_scene:     每个场景连续取 frames_per_scene 帧（21 场景 × N 帧）
      - single_scene:  只取 scene_id 对应场景的前 frames_per_scene 帧

    sampling_mode 仅在 per_scene 模式下生效：
      - "head":        从第 1 帧开始取
      - "skip_warmup": 从第 5 帧开始取，跳过 BEV 历史队列预热期
    """
    if split in ("test", "val"):
        # val 和 test 都使用 cfg.data.test 配置（与评估对齐）
    ...
    print(f"[INFO] 子集大小: {len(subset)} 帧（mode={subset_mode}, sampling_mode={sampling_mode}）")
...
# ======================== 评估对齐验证工具 ========================
# 从 verify_subset_eval.py 移植，确保推理输出与 dataset 的 token 一一对应。
# ===================================================================
...
def verify_subset_gt_alignment(eval_dataset, outputs, jsonfile_prefix):
    """验证子集评估的 GT 过滤是否与预测样本完全匹配。

    在 evaluate() 生成 results_nusc_det.json 之后调用，
    检查 pred tokens 和 GT tokens 是否完全一致。
    """
    import json as _json
    result_det_path = os.path.join(jsonfile_prefix, "results_nusc_det.json")
    if not os.path.exists(result_det_path):
        print(f"[VERIFY][WARN] 未找到 {result_det_path}，跳过 GT 对齐验证。")
        return

    with open(result_det_path, "r", encoding="utf-8") as f:
        result_det = _json.load(f)

    pred_tokens = set(result_det.get("results", {}).keys())
    ds_tokens = set(info["token"] for info in eval_dataset.data_infos)

    print(f"[VERIFY] dataset tokens: {len(ds_tokens)}, pred tokens: {len(pred_tokens)}")

    ds_minus_pred = ds_tokens - pred_tokens
    pred_minus_ds = pred_tokens - ds_tokens
    if ds_minus_pred:
        print(f"[VERIFY][WARN] dataset 有 {len(ds_minus_pred)} 个 token 未出现在 pred 中: "
              f"{_preview_tokens(ds_minus_pred)}")
    if pred_minus_ds:
        print(f"[VERIFY][WARN] pred 有 {len(pred_minus_ds)} 个 token 未出现在 dataset 中: "
              f"{_preview_tokens(pred_minus_ds)}")
    if not ds_minus_pred and not pred_minus_ds:
        print(f"[VERIFY] pred tokens 与 dataset tokens 完全一致。")


# ======================== score_func 调用追踪器 ========================
# 记录每次 score_func 调用的时间戳、得分、调用栈来源，
# 在剪枝完成后输出完整报告，证明 score_func 确实被 FastNAS 调用。
# =====================================================================

class ScoreFuncTracker:
    """追踪 score_func 的调用历史。"""
    def __init__(self):
        self.call_history = []   # [(call_id, timestamp, score, caller_info)]

    def record(self, call_id, score):
        import traceback as _tb
        # 提取调用栈中 ModelOpt 相关的帧，证明调用来源
    ...
    def print_report(self):
        print(f"\n{'='*70}")
        print(f"[TRACKER] score_func 调用报告")
        print(f"{'='*70}")
        print(f"[TRACKER] 总调用次数: {len(self.call_history)}")
        if not self.call_history:
            print("[TRACKER][WARN] score_func 从未被调用！")
            print("[TRACKER]       这意味着 FastNAS 没有使用你的评分函数来评估候选子网。")
            print("[TRACKER]       可能原因：搜索从 checkpoint 恢复且已完成。")
        else:
            scores = [h["score"] for h in self.call_history]
            print(f"[TRACKER] 得分范围: [{min(scores):.6f}, {max(scores):.6f}]")
            print(f"[TRACKER] 首次调用: {self.call_history[0]['timestamp']}")
            print(f"[TRACKER] 末次调用: {self.call_history[-1]['timestamp']}")
            print(f"[TRACKER] 调用明细:")
            for h in self.call_history:
                print(f"  #{h['call_id']}  {h['timestamp']}  score={h['score']:.6f}")
                if h["caller_info"]:
                    for line in h["caller_info"].split("\n"):
                        print(f"    {line}")
        print(f"{'='*70}\n")


# 全局 tracker 实例
_score_func_tracker = ScoreFuncTracker()

# --------------------------- score func ---------------------------
@torch.no_grad()
def _eval_car_mean_ap(model_dp: MMDataParallel, dataset, data_loader,
                      eval_kwargs: Dict[str, Any], call_id: int = 0) -> float:
    print(f"[score_func] >>>>>> ENTERING _eval_car_mean_ap (call #{call_id}), "
          f"dataset size={len(dataset)}, loader batches={len(data_loader)} <<<<<<", flush=True)
    model_dp.eval()
    outputs = single_gpu_test(model_dp, data_loader, show=False)
    print(f"[score_func] 推理完成，得到 {len(outputs)} 个样本结果", flush=True)

    # token 对齐校验
    assert_output_token_alignment(dataset, outputs)
    ...
    # 评估后验证 GT 对齐
    verify_subset_gt_alignment(dataset, outputs, eval_kwargs.get("jsonfile_prefix", ""))
...
def build_score_func(dataset, val_loader, eval_kwargs: Dict[str, Any], gpu_id: int = 0):
    ...
    def score_func(model: nn.Module) -> float:
        ...
        print(f"\n{'='*60}", flush=True)
        print(f"[score_func] >>>>>> CALLED (#{call_id}) <<<<<<", flush=True)
        ...
        real_model = real_model.to(f"cuda:{gpu_id}")
        model_dp = MMDataParallel(real_model, device_ids=[gpu_id])
        score = _eval_car_mean_ap(model_dp, dataset, val_loader, eval_kwargs, call_id=call_id)

        # 记录调用历史
        _score_func_tracker.record(call_id, score)

        print(f"[score_func] >>>>>> RETURNING score = {score:.6f} (call #{call_id}) <<<<<<\n",
              flush=True)
        return score
    return score_func
...
def main():
    ...
    print(f"[INFO] 构建 BN 校准集（split={args.calib_split}，"
          ...
          f"sampling_mode={args.calib_sampling_mode}"
    ...
    calib_dataset, calib_loader = build_subset_loader(
        ...
        sampling_mode=args.calib_sampling_mode,
    )
    ...
    print(f"[INFO] 构建评分集（split={args.score_split}，"
          ...
          f"sampling_mode={args.score_sampling_mode}"
    ...
    val_dataset, val_loader = build_subset_loader(
        ...
        sampling_mode=args.score_sampling_mode,
    )
    ...
    # ---- score_func 调用报告 ----
    _score_func_tracker.print_report()
